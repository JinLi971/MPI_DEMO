...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        sendbufrecv =  (double*) malloc(ROWS*COLS*sizeof(double));
        //set send buffer to a random number
        for (int ii=0; ii<ROWS*COLS; ii++) {
            sendbuf[ii] = ii;//drand48();
            sendbufrecv[ii] = 0;
        }
    }
    ...
    MPI_Scatterv(sendbuf, counts, disps, Pdatatype, recvbuf,
                 BLOCKROWS*BLOCKCOLS, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    ...
    for (int proc=0; proc<p; proc++) {
        if (proc == rank) {
            ...
            printf("Scatterv Local Matrix: at rank %d\n",rank);
    ...
    //gatherv
    MPI_Gatherv(recvbuf, BLOCKROWS*BLOCKCOLS, MPI_DOUBLE, sendbufrecv, counts, disps, Pdatatype,
                0, MPI_COMM_WORLD);

    if(rank ==0){
        int err = 0;
        for(int i  = 0; i<ROWS*COLS; i++){
            if(sendbufrecv [i] != sendbuf[i]){
                printf ("Gatherv %f %f \n", sendbuf[i], sendbufrecv[i]);
                err++;
            }
        }
        if (err !=0){
            printf("Gatherv error %d \n",err);
        }
        else{
            printf("Gather the right thing! \n");
        }
    }
    ...
    MPI_Irecv(recvbuf2, BLOCKROWS*BLOCKCOLS, MPI_DOUBLE ,
              left, 123, MPI_COMM_WORLD, &request);
    MPI_Isend(recvbuf,  BLOCKROWS*BLOCKCOLS, MPI_DOUBLE ,
              right, 123, MPI_COMM_WORLD, &request2);
    MPI_Wait(&request, &status);

    for (int proc=0; proc<p; proc++) {
        if (proc == rank) {
            printf("Rank = %d\n", rank);
            printf("Isend Irecv Local Matrix:\n");
            for (int ii=0; ii<BLOCKROWS; ii++) {
                for (int jj=0; jj<BLOCKCOLS; jj++) {
